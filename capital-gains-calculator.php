<?php
// =============================================================================
// TEMPLATE NAME: Capital Gains Calculator
// -----------------------------------------------------------------------------
// Calculate and report cryptocurrency capital gains for tax purposes
// =============================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'calculate_gains') {
        $tax_year = $_POST['tax_year'] ?? date('Y');
        $cost_basis_method = $_POST['cost_basis_method'] ?? 'FIFO';
        $selected_assets = $_POST['selected_assets'] ?? [];
        $include_short_term = isset($_POST['include_short_term']);
        $include_long_term = isset($_POST['include_long_term']);
        
        // Process transactions for capital gains
        $capital_gains = calculateCapitalGains($pdo, $tax_year, $cost_basis_method, $selected_assets, $include_short_term, $include_long_term);
        $summary = calculateCapitalGainsSummary($capital_gains);
        $message = "Capital gains calculated successfully for tax year {$tax_year}";
    }
}
...
function calculateCapitalGains($pdo, $tax_year, $cost_basis_method, $selected_assets, $include_short_term, $include_long_term) {
    ...
    foreach ($sales as $sale) {
        ...
        // Filter based on term preferences
        if (($is_long_term && !$include_long_term) || (!$is_long_term && !$include_short_term)) {
            continue;
        }
...
// Calculate cost basis for gains
...
function calculateCapitalGainsSummary($capital_gains) {
    $summary = [
    ...
    foreach ($capital_gains as $transaction) {
...
<div class="container">
    ...
    <div class="page-header">
        ...
        <p>Calculate and report your cryptocurrency capital gains for tax purposes</p>
    </div>
    ...
    <div class="form-container">
        <div class="form-header">
            <h3>⚙️ Calculate Capital Gains</h3>
        </div>
        
        <form method="POST" action="">
            <input type="hidden" name="action" value="calculate_gains">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <div class="form-help">Method used to calculate cost basis for sales</div>
                </div>
            </div>

            <!-- Term Selection -->
            <div class="term-selection">
                <h4>📅 Select Capital Gains Terms</h4>
                <div class="term-options">
                    <div class="term-option">
                        <input type="checkbox" name="include_short_term" id="include_short_term" 
                               <?php echo isset($_POST['include_short_term']) ? 'checked' : 'checked'; ?>>
                        <label for="include_short_term">Short-term Capital Gains (≤1 year)</label>
                    </div>
                    <div class="term-option">
                        <input type="checkbox" name="include_long_term" id="include_long_term" 
                               <?php echo isset($_POST['include_long_term']) ? 'checked' : 'checked'; ?>>
                        <label for="include_long_term">Long-term Capital Gains (>1 year)</label>
                    </div>
                </div>
            </div>
                    ...
                    <?php foreach ($summary['by_asset'] as $asset => $data): ?>
                        <div class="breakdown-item">
                            ...
                            <div class="breakdown-item-totals">
                                ...
                                <div class="breakdown-total-item">
                                    <div class="breakdown-total-label">Total Gains</div>
        ...
        <div class="gains-table-container">
            ...
            <div style="overflow-x: auto;">
                <table class="gains-table">
                    ...
                    <tbody>
                        <?php foreach ($capital_gains as $transaction): ?>
                        <tr>
...
<script>
    ...
    function exportToCSV() {
        const capitalGains = <?php echo json_encode($capital_gains ?? []); ?>;
        ...
        capitalGains.forEach(transaction => {
        ...
        csv += `Total Capital Gains,${summary.total_gain_loss}\n`;
        csv += `Short-term Gains,${summary.short_term_gain_loss}\n`;
        csv += `Long-term Gains,${summary.long_term_gain_loss}\n`;
        csv += `Winning Transactions,${summary.winning_transactions}\n`;
        csv += `Losing Transactions,${summary.losing_transactions}\n`;
        
        // Add asset breakdown
        csv += '\n\nAsset Breakdown\n';
        csv += 'Asset,Count,Proceeds,Cost Basis,Total Gains,Short-term,Long-term\n';
        ...
        a.href = url;
        a.download = `capital_gains_${document.getElementById('tax_year').value}.csv`;
