<?php
// =============================================================================
// TEMPLATE NAME: Advanced Form 8949 Generator
// -----------------------------------------------------------------------------
// Advanced Form 8949 generator with proper FIFO cost basis calculation
// =============================================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'generate_form') {
        ...
        $form_data = processTransactionsForForm8949Advanced($pdo, $tax_year, $form_type, $cost_basis_method);
        $summary = calculateForm8949SummaryAdvanced($form_data);
        $message = "Form 8949 generated successfully for tax year {$tax_year}";
    }
}

// Advanced transaction processing with proper cost basis calculation
function processTransactionsForForm8949Advanced($pdo, $tax_year, $form_type, $cost_basis_method) {
    ...
    // Process sales with cost basis calculation
    $processed_sales = [];
    $asset_inventory = [];
    ...
    foreach ($sales as $sale) {
        ...
        $cost_basis = calculateCostBasisAdvanced($asset, $amount_sold, $purchases, $cost_basis_method);
        ...
        $processed_sales[] = [
            ...
            'description' => "Sale of {$amount_sold} {$asset}",
            'proceeds' => $proceeds,
...
// Advanced summary calculation
function calculateForm8949SummaryAdvanced($form_data) {
    $short_term = [];
    $long_term = [];
...
<div class="container">
    ...
    <div class="page-header">
        <h1>📋 Advanced Form 8949 Generator</h1>
        <p>Generate IRS Form 8949 with advanced cost basis calculation methods</p>
    </div>
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    ...
                    <select name="cost_basis_method" id="cost_basis_method" class="form-control">
                        <option value="FIFO" <?php echo $cost_basis_method == 'FIFO' ? 'selected' : ''; ?>>FIFO (First In, First Out)</option>
                        <option value="LIFO" <?php echo $cost_basis_method == 'LIFO' ? 'selected' : ''; ?>>LIFO (Last In, First Out)</option>
                        <option value="AVERAGE" <?php echo $cost_basis_method == 'AVERAGE' ? 'selected' : ''; ?>>Average Cost</option>
                    </select>
                    <div class="form-help">Choose how to calculate cost basis for your transactions</div>
                </div>
            </div>
    ...
    <?php if ($form_data && $summary): ?>
        ...
        <div class="method-info">
            <h4>📊 Cost Basis Method: <?php echo $cost_basis_method; ?></h4>
            <p>
                <?php if ($cost_basis_method === 'FIFO'): ?>
                    Using First In, First Out method - oldest purchases are sold first.
                <?php elseif ($cost_basis_method === 'LIFO'): ?>
                    Using Last In, First Out method - newest purchases are sold first.
                <?php else: ?>
                    Using Average Cost method - cost basis calculated as average of all purchases.
                <?php endif; ?>
            </p>
        </div>
        ...
        <div class="summary-grid">
            ...
            <div class="summary-card">
                ...
                <div class="summary-subtitle">Using <?php echo $cost_basis_method; ?> method</div>
            </div>
            <div class="summary-card">
                <h4>Net Gain/Loss</h4>
...
<script>
    ...
    function exportToCSV() {
        ...
        let csv = 'Date Acquired,Date Sold,Description,Proceeds,Cost Basis,Adjustment Code,Adjustment Amount,Gain/Loss,Asset,Amount,Transaction Type\n';
        
        formData.forEach(transaction => {
            csv += `"${transaction.date_acquired}","${transaction.date_sold}","${transaction.description}","${transaction.proceeds}","${transaction.cost_basis}","${transaction.adjustment_code}","${transaction.adjustment_amount}","${transaction.gain_loss}","${transaction.asset}","${transaction.amount}","${transaction.is_short_term ? 'Short Term' : 'Long Term'}"\n`;
        });
