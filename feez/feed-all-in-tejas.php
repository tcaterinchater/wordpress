...
//For page links
 $html = file_get_html('http://www.pbs.org/wgbh/frontline/topic/health/page/2/');
		...
		$rssdata = array();
				// Find all links
		$keycount = 0;
		foreach($html->find('div[class=list__img] img') as $element){
			   echo $url = strtok($element->src, '?');
			   echo "<br>";
				header("Content-type:image/jpg");

// It will be called downloaded.pdf
header("Content-Disposition:attachment;filename=image".$keycount.".jpg");

// The PDF source is in original.pdf
readfile($url);
			   $rssdata[$keycount]['link'] = $url;
			   $rssdata[$keycount]['count'] = $keycount;
			   $keycount++;
		}
		$key = 0;
		/*foreach($rssdata as $data_desc){
			if(in_array($data_desc['count'],range(501,600))) {
...
?>
	...
	<channel>
		...
		<?php foreach($rssdata as $rdata){
			if(in_array($rdata['count'],range(0,99))) { ?>
		<item>
