...
class Scraper{
  ...
  function __construct($startIndex = 0, $endIndex = 100){
    ...
    $this->page = $this->browser->get('http://www.colbertnation.com/video');
  ...
  function process($page){
    ...
    foreach($page->search('div.video_result') as $div){
      $item = array();
      $item['title'] = $page->at('a.clipTitle', $div)->text();
      $item['tags'] = implode(array_map(function($x){return $x->text();}, $page->search('p.tags a', $div)), ', ');
      $item['description'] = trim($page->at('p.description', $div)->text());
      ...
      $str = $page->at('div.meta span.posted', $div)->text();
  ...
  function nextItem(){
    ...
    # do paging
    $nextLink = $this->page->at('a.next');

    # stop if there's no more
    if(preg_match('/disabled/', $nextLink->class)) return null;

    if(!preg_match("/'(.*)'/", $nextLink->onclick, $m)) die('</channel></rss>');
    $nextUrl = $m[1];
